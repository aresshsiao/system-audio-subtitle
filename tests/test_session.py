...
def test_higher_revision_replaces_lower_for_same_utterance() -> None:
    session = Session()
    session.apply(make_transcript("u1", 0, "hel"))
    t2 = make_transcript("u1", 1, "hello")
...
def test_stale_revision_is_ignored() -> None:
    ...
    session = Session()
    session.apply(make_transcript("u1", 2, "hello world"))
    changed = session.apply(make_transcript("u1", 1, "hello"))  # 過期的舊訊息晚到

    assert changed is False
    assert session.latest("u1").text == "hello world"  # 沒有被舊訊息覆蓋


def test_new_utterance_becomes_display_focus_even_if_previous_not_final() -> None:
    """新句子一開始，顯示焦點就切過去——即使前一句還沒收到 FINAL。"""
    session = Session()
    session.apply(make_transcript("u1", 0, "first"))
    session.apply(make_transcript("u2", 0, "second"))

    assert session.current_display().utt_id == "u2"
    assert session.latest("u1").text == "first"  # u1 的狀態還在，只是不是顯示焦點


def test_final_utterance_stays_displayed_until_next_utterance_starts() -> None:
    session = Session()
    session.apply(make_transcript("u1", 0, "draft text", state=SubtitleState.DRAFT))
    final = make_transcript("u1", 1, "final text", state=SubtitleState.FINAL)
    session.apply(final)
...
def test_history_preserves_first_seen_order_with_latest_state() -> None:
    session = Session()
    session.apply(make_transcript("u1", 0, "a"))
    session.apply(make_transcript("u2", 0, "b"))
    session.apply(make_transcript("u1", 1, "a-updated"))  # u1 更新，但順序不變

    history = session.history()
    assert [t.utt_id for t in history] == ["u1", "u2"]
    assert history[0].text == "a-updated"


def test_clear_resets_everything() -> None:
    session = Session()
    session.apply(make_transcript("u1", 0, "hello"))
    session.clear()
